	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			#if sys
			if (#if MODS_ALLOWED mobile.Util.exists(Paths.modsJson(songName + '/events')) || #end mobile.Util.exists(file))
			#else
		...
		for (i in 0...directories.length) {
			...
			if(mobile.Util.exists(directory)) {
				for (file in mobile.Util.readDirectory(directory)) {
					var path = haxe.io.Path.join([directory, file]);
					if (!mobile.Util.isDirectory(path) && file.endsWith('.json')) {
	...
	function addNoteUI():Void
	{
		...
		for (i in 0...directories.length) {
			...
			if(mobile.Util.exists(directory)) {
				for (file in mobile.Util.readDirectory(directory)) {
					var path = haxe.io.Path.join([directory, file]);
					if (!mobile.Util.isDirectory(path) && file.endsWith('.lua')) {
	...
	function addEventsUI():Void
	{
		...
		for (i in 0...directories.length) {
			...
			if(mobile.Util.exists(directory)) {
				for (file in mobile.Util.readDirectory(directory)) {
					var path = haxe.io.Path.join([directory, file]);
					if (!mobile.Util.isDirectory(path) && file != 'readme.txt' && file.endsWith('.txt')) {
						var fileToCheck:String = file.substr(0, file.length - 4);
	...
	function loadAudioBuffer() {
		...
		audioBuffers[0] = null;
		#if MODS_ALLOWED
		if(mobile.Util.exists(Paths.modFolders('songs/' + currentSongName + '/Inst.ogg'))) {
		...
		audioBuffers[1] = null;
		#if MODS_ALLOWED
		if(mobile.Util.exists(Paths.modFolders('songs/' + currentSongName + '/Voices.ogg'))) {
